...
class GameRunner:
    ...
    def train(self, robot, prophet,  mBuffer, batch_size, initial_lr_1, epoch,  device1):
        optimizer1 = optim.Adam(robot.pnet.parameters(), lr=initial_lr_1, betas=(0.9, 0.999), eps=1e-04, weight_decay=0.00001, amsgrad=False)
        #optimizer1 = base_optimizer1 #torchcontrib.optim.SWA(base_optimizer1, swa_start=1, swa_freq=5, swa_lr=initial_lr_1)
        #optimizer1 = optim.SGD(robot.pnet.parameters(), lr=initial_lr_1, momentum=0.5)
...
from arenaoffline.arena import res_vs_if
if __name__ == '__main__':
    ...
    if LOAD_MODEL:
        #p4.load_state_dict(
        #    torch.load(r"C:\Users\SuperLi\Desktop\Shi\game\wuji\version5supervised\rawdata\robot-net-s4.txt"), False)

        p4.load_state_dict(torch.load(r"robot-net.txt"),False)
        p4.eval()
        print('model loaded')
    v4 = VNet()

    score0, score1 = res_vs_if(p4, 100)
    print('s0: '+str(score0.mean())+ 's1: '+str(score1.mean()))
